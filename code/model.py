...
class MultiModalClassifier(nn.Module):
    def __init__(self, input_size=768, num_classes=4, 
        ...
        self.token_type_embeddings = nn.Embedding(2, input_size)#将索引转换为向量，建立一个查表矩阵 [2, input_size]
    ...
    def forward(self, eeg_data, images_data):
        ...
        if self.fusion_type == 'cross_attention':
            ...
            images_embedding = image_features  # [batch_size, 147, 768]
        ...
        images_embedding, eeg_embedding = (
            images_embedding + self.token_type_embeddings(torch.zeros(images_embedding.shape[0], 1, dtype=torch.long, device=device)),#(batch_size,1) 全为0
